...
// Obtener las categorías únicas de los eventos pasados
const pastEvents = MockData.events.filter(
    (event) => new Date(event.date) < new Date(MockData.currentDate)
);
...
// Mostrar categorías en los checkboxes
...
const showEvents = (data) => {
    ...
    data.events.forEach((event) => {
        if (new Date(event.date) < new Date(MockData.currentDate)) {
...
const applyFilterSearch = (categories = [], textSearch = "") => {
    ...
    if (categories.length > 0 || textSearch) {
        result.events = pastEvents.filter(
            (event) =>
                (categories.length === 0 ||
        ...
        result.events = pastEvents;
    }
    showEvents(result);
};

document.addEventListener("DOMContentLoaded", () => {
    showEvents({ events: pastEvents });
