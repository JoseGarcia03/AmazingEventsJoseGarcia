...
// Print categories
const categories = [...new Set(MockData.events.map((event) => event.category))];
...
const showEvents = (data) => {
    ...
    if (data.events.length === 0) {
        ...
        noResultsMessage.textContent =
            "No se han encontrado eventos que coincidan con su busqueda";
...
const applyFilterSearch = (categories = [], textSearch = "") => {
    ...
    if (categories.length > 0 || textSearch) {
        result.events = MockData.events.filter(
            (event) =>
                (categories.length === 0 ||
        ...
        result.events = MockData.events;
    }
    showEvents(result);
};

document.addEventListener("DOMContentLoaded", () => {
    showEvents({ events: MockData.events });
